...
class QuestionAnswering(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,
                start_probs=None, end_probs=None,
                ve_value=None):
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        # extended_attention_mask = extended_attention_mask.to(torch.float32)
        ...
        if start_probs is not None and end_probs is not None:
            ...
            KD_loss1 = torch.sum(-F.softmax(start_probs/T, dim=1) * F.log_softmax(start_logits/T, dim=1), -1)
            KD_loss2 = torch.sum(-F.softmax(end_probs/T, dim=1) * F.log_softmax(end_logits/T, dim=1), -1)

            total_loss = (KD_loss1 + KD_loss2) / 2 * alpha + (start_loss + end_loss) / 2 * (1 - alpha)
            return total_loss
        else:
